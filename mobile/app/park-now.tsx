...
import { getPricing } from '../src/api';
...
const PARK_ACTION_MODE = 'park' as const;

/** Giriş yapmış kullanıcıda alan seçimi / QR adımı için üst süre */
const PARK_NOW_IDLE_MS = 5 * 60 * 1000;

export default function ParkNowScreen() {
  const { token } = useAuth();
  ...
  // Sadece mount'ta bir kez: provider'da kalan eski seçim state'ini temizle.
  // useFocusEffect içine konulamaz, çünkü hasSelectedSpot dep'i değişince yeniden fire eder.
  useEffect(() => {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      clearParkNowIdleTimer();
      const deadline = Date.now() + PARK_NOW_IDLE_MS;
  ...
  // Idle countdown rengi: > 2dk yeşil, 1-2dk sarı, <1dk kırmızı
  const idleUrgency: 'safe' | 'warn' | 'danger' =
    idleDisplaySec > 120 ? 'safe' : idleDisplaySec > 60 ? 'warn' : 'danger';

  // Akış adımı: alan seçildiyse 'details', araç+kart varsa 'summary'
  ...
  const showEntryQrFooter = !!token && !activeSession;
  const scrollBottomPad = showEntryQrFooter ? spacing.lg : bottomPad;
  ...
  const planParkingDisplay = useMemo(() => {
    ...
    if (uiSelectedSpotId != null) {
      ...
      if (spot) {
        return {
          ...
          hint: 'Seçili alan; parkı bir sonraki adımda başlatacaksınız',
        };
      }
    }
    ...
    if (activeReservation) {
      ...
      return {
        ...
        hint: 'Rezervasyonunuz var — üstteki Park Et veya alttaki Giriş QR’ı ile süreyi başlatın',
      };
    }
  ...
  useEffect(() => {
    ...
    // Cache: zaten alınmış pricing varsa tekrar çekme
  ...
  // Seçili spot, aktif bölümden farklı bir bölüme aitse temizle.
  // Section field stabil olduğu için polling'de gelen yeni filteredSpots ref'i
  // boşa fire etmez — yalnızca gerçek bölüm değişikliklerinde tetiklenir.
  useEffect(() => {
    if (uiSelectedSpotId == null) return;
    const selected = spotList.find((s) => s.id === uiSelectedSpotId);
    if (!selected) return; // henüz yüklenmemiş olabilir; temizleme
    const spotSection = selected.section?.trim() || 'Genel';
    if (spotSection !== selectedSectionKey) {
      setUiSelectedSpotId(null);
    }
  }, [selectedSectionKey, uiSelectedSpotId, spotList, setUiSelectedSpotId]);
  ...
  useEffect(() => {
    ...
    // Aktif rezervasyon varsa öneri rezerve edilen spota zorlanır
    const sid =
      activeReservation && activeReservation.spot_id !== pendingSuggestedSpotId
        ? activeReservation.spot_id
        : pendingSuggestedSpotId;
    const spot = spots.spots.find((s) => s.id === sid);
    setPendingSuggestedSpotId(null);

    if (!spot) return;
    const isOwnReservation = activeReservation?.spot_id === spot.id;
    if (spot.is_occupied || (spot.is_reserved && !isOwnReservation)) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: pageBg }]}>
      <ScrollView
        ...
        nestedScrollEnabled
      >
        {token && !activeSession ? <FlowProgress current={flowStep} mode="park" /> : null}
        ...
        <ParkingLotPanels
          ...
          onSpotPress={(spot) => handleSpotPress(spot, PARK_ACTION_MODE)}
                ...
                <View style={[styles.formCardInset, { marginBottom: spacing.md }]}>
                  ...
                  >
                    ...
                    <View style={styles.idleCountdownTextCol}>
                      ...
                      <Text style={[styles.muted, { color: colors.textSecondary }]}>
                        {idleUrgency === 'danger'
                          ? 'Süreniz dolmak üzere! Hemen alan seçin veya QR ekranına geçin.'
                          : 'Alan seçin veya giriş QR’ına geçin; süre dolunca ana sayfaya yönlendirilirsiniz.'}
                      </Text>
                    </View>
                  </View>
                </View>
            ...
            <View style={styles.blockBelowParking}>
              ...
              <View style={[styles.formCardInset, { marginBottom: spacing.md }]}>
                ...
                >
                  ...
                  >
                    <Ionicons name="qr-code-outline" size={22} color={colors.warningDark} />
                    <Text style={[styles.planQrWarningText, { color: colors.warningDark }]}>
                      Giriş QR kodunu okuttuğunuzda süreniz başlatılacaktır.
                    </Text>
                  </View>
                </View>
              </View>
      ...
      {showEntryQrFooter ? (
        ...
        >
          {effectiveSpotId != null ? (
            <TouchableOpacity
              style={[styles.entryQrFooterBtn, { backgroundColor: colors.brandDeep }]}
              onPress={() => navigateToParkEntryQr(effectiveSpotId)}
              activeOpacity={0.85}
              accessibilityRole="button"
              accessibilityLabel="Giriş QR'ını göster"
            >
              <Text style={styles.entryQrFooterBtnText}>Giriş QR’ını göster</Text>
            </TouchableOpacity>
          ) : (
            <Text style={[styles.entryQrFooterHint, { color: colors.textSecondary }]}>
              Önce bir alan seçin veya rezervasyonunuzu kullanın; ardından giriş QR’ını gösterebilirsiniz.
            </Text>
          )}
        </View>
      ) : null}
      ...
      >
        <View style={styles.priceModalBackdrop}>
          ...
          >
            ...
            <Text style={[styles.priceModalSubtitle, { color: colors.textSecondary }]}>
              5 dakika içinde park yeri seçilmedi veya giriş QR’ı adımına geçilmedi. Ana sayfaya
